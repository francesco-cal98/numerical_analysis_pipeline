...
def _curvature_report(name: str, X: np.ndarray, y: np.ndarray, outdir: Path, run_isomap: bool) -> CurvatureStats:
    ...
    plt.plot(range(2, len(traj)), curv, marker="o", lw=2, markersize=6, color="tab:blue")
    plt.axhline(c_mean, ls="--", lw=1.5, color="red", alpha=0.7, label=f"Mean = {c_mean:.3f}")
    plt.title("PCA-3D Curvature Along Numerosity Centroids", fontsize=12, fontweight="bold")
    plt.xlabel("Numerosity Index", fontsize=11)
    plt.ylabel(r"Curvature $||x_{i+1} - 2x_i + x_{i-1}||$", fontsize=11)
    plt.grid(alpha=0.3, linestyle=":")
    plt.legend(fontsize=10)
    plt.tight_layout()
    outdir.mkdir(parents=True, exist_ok=True)
    plt.savefig(outdir / f"curvature_centroids_{name}.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    if run_isomap:
        try:
            ...
            plt.figure(figsize=(6, 4))
            plt.plot(range(len(iso_traj)), iso_traj[:, 0], marker="o", lw=2, markersize=6, color="tab:green")
            plt.title("Isomap Embedding (Dimension 1)", fontsize=12, fontweight="bold")
            plt.xlabel("Numerosity Index", fontsize=11)
            plt.ylabel("Isomap Component 1", fontsize=11)
            plt.grid(alpha=0.3, linestyle=":")
            plt.tight_layout()
            plt.savefig(outdir / f"isomap_dim0_{name}.png", dpi=300, bbox_inches="tight")
            plt.close()
        except Exception:
            pass
...
def _save_variance_plots(name: str, eig: np.ndarray, outdir: Path):
    ...
    plt.figure(figsize=(6, 4))
    plt.plot(range(1, len(eig_ratio) + 1), eig_ratio, marker="o", lw=2, markersize=6, color="tab:blue")
    plt.title("Explained Variance Ratio by Component", fontsize=12, fontweight="bold")
    plt.xlabel("Principal Component", fontsize=11)
    plt.ylabel("Variance Ratio", fontsize=11)
    plt.ylim(0, max(eig_ratio) * 1.1)
    plt.grid(alpha=0.3, linestyle=":")
    plt.tight_layout()
    outdir.mkdir(parents=True, exist_ok=True)
    plt.savefig(outdir / f"eigenvalue_ratios_{name}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def _save_angle_plot(name: str, angles: AngleStats, outdir: Path):
    plt.figure(figsize=(6, 4))
    components = ["PC1", "PC2"]
    values = [angles.angle_pc1_deg, angles.angle_pc2_deg]
    colors = ["tab:blue", "tab:orange"]
    bars = plt.bar(components, values, color=colors, width=0.6, edgecolor="black", linewidth=1.2)
    ...
    plt.title("Alignment with Numerosity Axis", fontsize=12, fontweight="bold")
    plt.ylim(0, 90)
    plt.grid(axis="y", alpha=0.3, linestyle=":")
    plt.tight_layout()
    outdir.mkdir(parents=True, exist_ok=True)
    plt.savefig(outdir / f"angles_{name}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def _save_balanced_plot(name: str, balanced: BalancedPCAStats, outdir: Path):
    plt.figure(figsize=(6, 4))
    components = ["PC1", "PC2"]
    values = [balanced.rho_pc1, balanced.rho_pc2]
    ...
    plt.ylabel("Spearman ρ", fontsize=11)
    plt.title("Balanced Correlation with Numerosity", fontsize=12, fontweight="bold")
    plt.ylim(-1, 1)
    plt.axhline(0, color="black", linewidth=0.8, linestyle="-", alpha=0.3)
    plt.grid(axis="y", alpha=0.3, linestyle=":")
    plt.tight_layout()
    outdir.mkdir(parents=True, exist_ok=True)
    plt.savefig(outdir / f"balanced_corr_{name}.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    *,
    ...
    for d in components:
        ...
        if d == 2:
            ...
            sc = plt.scatter(scores[:, 0], scores[:, 1], c=y, cmap="viridis",
                           s=15, alpha=0.7, edgecolors="none")
            plt.xlabel("PC1", fontsize=11)
            plt.ylabel("PC2", fontsize=11)
            plt.title("PCA Projection (2D)", fontsize=12, fontweight="bold")
            cbar = plt.colorbar(sc, label="Numerosity")
            cbar.set_label("Numerosity", fontsize=11)
            plt.grid(alpha=0.2, linestyle=":")
            plt.tight_layout()
            plt.savefig(outdir / f"projection_{name}_dim{d}.png", dpi=300, bbox_inches="tight")
            plt.close()
        elif d == 3:
            fig = plt.figure(figsize=(8, 6))
            ax = fig.add_subplot(111, projection="3d")
            sc = ax.scatter(scores[:, 0], scores[:, 1], scores[:, 2], c=y,
            ...
            ax.set_title("PCA Projection (3D)", fontsize=12, fontweight="bold", pad=15)
            cbar = plt.colorbar(sc, ax=ax, label="Numerosity", shrink=0.8, pad=0.1)
            cbar.set_label("Numerosity", fontsize=11)
            ax.grid(alpha=0.2)
            plt.tight_layout()
            plt.savefig(outdir / f"projection_{name}_dim{d}.png", dpi=300, bbox_inches="tight")
